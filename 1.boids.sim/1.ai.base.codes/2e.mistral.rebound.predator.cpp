...
DataFrame update_boids_cpp(
  ...
  // Get the number of boids and predators
  ...
  for (int i = 0; i < n_boids; i++) {
    ...
    NumericVector sep(2), ali(2), coh(2), pred(2); // sep=separation, ali=alignment, coh=cohesion, pred=predator avoidance
    sep[0] = sep[1] = ali[0] = ali[1] = coh[0] = coh[1] = pred[0] = pred[1] = 0.0; // Initialize to zero
...
DataFrame update_predators_cpp(
  ...
  for (int i = 0; i < n_predators; i++) {
    ...
    if (speed > max_speed * 1.2) { // Predators can move slightly faster
      pvx[i] = pvx[i] / speed * max_speed * 1.2;
      pvy[i] = pvy[i] / speed * max_speed * 1.2;
    }
